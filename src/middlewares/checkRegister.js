...
export async function checkRegister (ctx, next) {
  const body = ctx.request.body
  const { id, password, role, phone, identityNumber } = body.user

  if (!validPassword(password)) ctx.throw(404, '패스워드를 확인해주세요.', { code: 'S9999' })
  if (!validID(id)) ctx.throw(404, '아이디에 사용할 수 없는 문자가 포함되었습니다.', { code: 'S9999' })
  ...
  if (!await checkID(id)) ctx.throw(404, '동일한 아이디가 이미 존재합니다.', { code: 'S9999' })
